...
# ============================
# Simple 5-layer FCN Definition
# (Using the FCN_modified architecture)
# ============================
class FCN_modified(nn.Module):
    def __init__(self, in_channels: int, classes: int, num_filters: int = 64):
        super(FCN_modified, self).__init__()
...
# ============================
# Composite Model: FCN + U-Net
# ============================
class CompositeModel(nn.Module):
    def __init__(self, fcn_out_channels: int, unet_features=[64, 128, 256, 512]):
        """
        Composite model that processes the additional channels with an FCN and then concatenates its output
        with the RGB channels before feeding into U-Net.
        
        Args:
            fcn_out_channels: Number of output channels from the FCN.
            unet_features: Feature sizes for U-Net.
        """
        super(CompositeModel, self).__init__()
        # For input of 23 channels, assume first 3 are RGB and the remaining 20 are extra.
        # Here we process the extra 20 channels with the FCN.
        self.fcn = FCN_modified(in_channels=20, classes=fcn_out_channels, num_filters=64)
        # U-Net takes concatenated features: RGB (3 channels) + FCN output (fcn_out_channels).
        self.unet = UNet(in_channels=3 + fcn_out_channels, out_channels=1, features=unet_features)
    
    def forward(self, x):
        # x shape: (B, 23, H, W)
        rgb = x[:, :3, :, :]
        extra = x[:, 3:, :, :]  # extra channels: shape (B, 20, H, W)
        fcn_out = self.fcn(extra)  # shape: (B, fcn_out_channels, H, W)
        composite = torch.cat([rgb, fcn_out], dim=1)  # shape: (B, 3 + fcn_out_channels, H, W)
        out = self.unet(composite)
        return out
...
class HDF5Dataset(Dataset):
    def __init__(self, hdf5_path, split='train', transform=None, channels="all"):
        ...
        The images are assumed to have shape (N, C, H, W) where:
          - For channels=="rgb": C = 3.
          - For channels=="all": C = 23 (3 RGB, 19 OSM, 1 DEM).
        Pre-computed channel-wise normalization is applied for the RGB channels,
        and the additional channels are left unnormalized.
        """
    ...
    def __getitem__(self, idx):
        # Load pre-augmented image (already in channels-first format)
        image = self.images[idx].astype(np.float32)  # shape: (C, H, W), where C=3 or 23.
        mask = self.masks[idx]
        if mask.ndim == 3:
            mask = np.mean(mask, axis=2, keepdims=True)
        mask = mask.astype(np.float32)
        
        # Normalize only the RGB channels if using all channels.
        if self.channels == "rgb":
            ...
            # For full composite (23 channels), normalize only the first three channels (RGB)
            channel_means = np.array([61.18, 75.86, 81.15], dtype=np.float32)
            channel_stds  = np.array([48.14, 31.38, 29.15], dtype=np.float32)
            for c in range(3):  # Normalize only RGB channels
                image[c] = (image[c] - channel_means[c]) / channel_stds[c]
            # Leave channels 3 to 22 unchanged.
        
        if self.transform:
            image = self.transform(image)
        image_tensor = torch.from_numpy(image)  # shape: (C, H, W)
        mask_tensor = torch.from_numpy(mask).permute(2,0,1)  # shape: (1, H, W)
...
def visualize_predictions(model, dataloader, device, channels_mode):
    ...
    for i in range(inputs.shape[0]):
        ...
        # For visualization, we always show the first three channels as RGB.
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Training script with FCN ablation for extra channels")
    ...
    parser.add_argument('--channels', type=str, default='rgb', choices=['rgb', 'all'], help='Use only RGB channels or full composite (23 channels)')
    parser.add_argument('--fcn_out_channels', type=int, default=1, help='Number of output channels for the FCN (used when channels=="all")')
    ...
    train_hdf5 = '/home/arra4944/projects/sustainbench/raw_id_augmented_train_split_with_osm_new.h5'
    val_hdf5   = '/home/arra4944/projects/sustainbench/raw_id_augmented_test_split_with_osm_new.h5'
    test_hdf5  = '/home/arra4944/projects/sustainbench/raw_id_augmented_val_split_with_osm_new.h5'
    ...
    # Create training and validation datasets (only subset the training set).
    ...
    # Initialize model.
    # If channels=="rgb", use standard U-Net with 3 input channels.
    # If channels=="all", use CompositeModel (FCN + U-Net).
    if args.channels == "rgb":
        model = UNet(in_channels=3, out_channels=1).to(device)
    else:
        model = CompositeModel(fcn_out_channels=args.fcn_out_channels).to(device)
    ...
    wandb.init(project="sustainbench-farm-ood-newosm-ablation")
    wandb.log({'subset_size': args.subset_fraction, 'seed': args.seed, 'channels': args.channels,
               'fcn_out_channels': args.fcn_out_channels})
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'best_composite_model.pth')
    ...
    if args.channels == "rgb":
        ...
        best_model = CompositeModel(fcn_out_channels=args.fcn_out_channels).to(device)
    best_model.load_state_dict(torch.load('best_composite_model.pth'))
    best_model.eval()
