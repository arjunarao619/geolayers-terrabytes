...
class HDF5Dataset(Dataset):
    def __init__(self, hdf5_path, split='train', transform=None, channels="all"):
        ...
        self.channels = channels.lower()  # 'rgb', 'all', 'osm', or 'dem'
    ...
    def __getitem__(self, idx):
        ...
        # Select channels based on mode and normalize
        if self.channels == "rgb":
            image = image[:3]
            channel_means = np.array([61.18, 75.86, 81.15], dtype=np.float32)
            channel_stds  = np.array([48.14, 31.38, 29.15], dtype=np.float32)
        elif self.channels == "osm":
            image = image[:6]  # RGB (0-2) + OSM (3-5)
            channel_means = np.array([61.18, 75.86, 81.15, 0.1545, 0.09379, 0.09237], dtype=np.float32)
            channel_stds  = np.array([48.14, 31.38, 29.15, 0.20566, 0.13182, 0.13612], dtype=np.float32)
        elif self.channels == "dem":
            image = np.stack([image[0], image[1], image[2], image[6]], axis=0)  # RGB + DEM
            channel_means = np.array([61.18, 75.86, 81.15, 271.83], dtype=np.float32)
            channel_stds  = np.array([48.14, 31.38, 29.15, 287.20], dtype=np.float32)
        else:
            # 'all'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--channels', type=str, default='rgb', choices=['rgb','all','osm','dem'],
                        help='Use RGB, all 7 channels, RGB+OSM (osm), or RGB+DEM (dem)')
    ...
    if args.channels == 'rgb':
        in_ch = 3
    elif args.channels == 'osm':
        in_ch = 6
    elif args.channels == 'dem':
        in_ch = 4
    else:
        in_ch = 7
    ...
    wandb.init(project="sustainbench-farm-id-isolate")
